...
* **Using DigitalOcean for DNS management.** ACME challenges normally rely on setting up a temporary web server, but the DNS ACME challenge (required for wildcard certificates) requires setting a TXT DNS records. Thus, we need a restricted DigitalOcean token that gives access only to DNS settings.
...
* **Using GitHub as code forge.** The primary lock-in here comes from use of GitHub Actions for CI/CD. Additionally, GitHub Container Registry is used for Minio (only used for integration testing in GitHub Actions). The git part of this forge is the most replaceable, as the Wikijump codebase is a standard git repository, and Komodo has good support for other forges.

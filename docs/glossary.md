...
| Canonical domain |            |             | The domain for a site which contains its site slug on Wikidot/Wikijump. For instance, `scp-wiki.wikijump.com`. Every site has exactly one canonical domain. (However, a site _may_ have multiple _aliases_, which always take the form of canonical domains. |
| Custom domain    |            |             | Any non-Wikidot/Wikijump domain used to serve site data on. For instance, `scpwiki.com` or `example.com`. A site may have multiple custom domains. |
| Preferred domain |            |             | Which domain is preferred for a site. Any domains used to access the site other than this one will be redirected to it. The preferred domain could be a custom domain or the canonical domain. In Wikidot only, a site may opt to not have a preferred domain, meaning all valid domains can be used to access the site. |
| Layout         |              |             | The HTML structure in which the site content is formatted. |